...
\begin{itemize}
	...
	\item[2)]
	Finding a set of functions(also called model, dependent on a set of parameters) that is used to represent the law behind the data set;
	\item[3)]
	Defining a loss function to measure the badness of your function, given certain parameter value, and find the best parameter value to minimize the loss function.
\end{itemize}
Considerations on each step can lead to methods with very different models and task-oriented algorithms.


\section[Basic Notion \& Application]{Basic notation \& Application\cite{{melko2016prb},{fischer2012tb}}}
